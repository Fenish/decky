...
String hex(const String &value){
    String result;result.reserve(value.length()*2);
    for(size_t i=0;i<value.length();++i){char b[3];snprintf(b,sizeof(b),"%02x",static_cast<uint8_t>(value[i]));result+=b;}
    return result.length()?result:String("-");
}
bool unhex(const char *value,String &result,size_t maximum){
    result="";if(strcmp(value,"-")==0)return true;
    const size_t length=strlen(value);if(length%2||length>maximum*2)return false;
    for(size_t i=0;i<length;i+=2){if(!isxdigit(value[i])||!isxdigit(value[i+1]))return false;char pair[3]={value[i],value[i+1],0};char byte=static_cast<char>(strtol(pair,nullptr,16));if(!byte)return false;result+=byte;}
    return true;
}
String random_hex(){uint8_t data[32];esp_fill_random(data,sizeof(data));String result;for(auto byte:data){char b[3];snprintf(b,sizeof(b),"%02x",byte);result+=b;}return result;}
String proof(const String &message){
    uint8_t digest[32];
    mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),reinterpret_cast<const uint8_t*>(secret.c_str()),secret.length(),reinterpret_cast<const uint8_t*>(message.c_str()),message.length(),digest);
    String result;for(auto byte:digest){char b[3];snprintf(b,sizeof(b),"%02x",byte);result+=b;}return result;
}
bool equal_secret(const String &a,const String &b){if(a.length()!=b.length())return false;uint8_t difference=0;for(size_t i=0;i<a.length();++i)difference|=a[i]^b[i];return difference==0;}
...
void session_key(const char *label,uint8_t key[16]){
    uint8_t digest[32];const String message=String(label)+nonce;
    mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),reinterpret_cast<const uint8_t*>(secret.c_str()),secret.length(),reinterpret_cast<const uint8_t*>(message.c_str()),message.length(),digest);
    memcpy(key,digest,16);
}
void drop_client(){
    client.stop();secure.reset();authenticated=false;net_length=0;net_overflow=false;
...
void read_handshake(){
    while(client.available()){
        ...
        if(byte=='\r'||byte=='\n'){
            ...
            net_line[net_length]=0;net_length=0;
...
void read_commands(Stream &stream,char *buffer,size_t &length,bool &overflow,void (*command)(const char *)){
    while(stream.available()){
        ...
        if(byte=='\r'||byte=='\n'){
            if(overflow){stream.println("ERR command too long");overflow=false;length=0;continue;}
            if(!length)continue;buffer[length]=0;length=0;
            source=&stream;
            command(buffer);
            source=&Serial;
        }else if(length<255)buffer[length++]=byte;else overflow=true;
    }
}
}
...
bool handle(const char *line){
    ...
    if(strcmp(line,"WIFI_STATUS")==0){
        const char *state=WiFi.status()==WL_CONNECTED?"connected":joining?"connecting":prefs.getString("ssid").length()?"disconnected":"unconfigured";
        reply().printf("OK wifi status=%s ssid=%s ip=%s saved=%d\n",state,hex(WiFi.status()==WL_CONNECTED?WiFi.SSID():joining_ssid).c_str(),WiFi.localIP().toString().c_str(),prefs.getString("ssid").length()?1:0);return true;
    }
    if(source!=&Serial){reply().println("ERR connect USB to configure Wi-Fi");return true;}
    if(strcmp(line,"WIFI_PAIR")==0){reply().printf("OK pair %s\n",secret.c_str());return true;}
    ...
    if(strcmp(line,"WIFI_LIST")==0){
        const int count=WiFi.scanComplete();if(count==WIFI_SCAN_RUNNING){reply().println("OK scanning");return true;}
        if(count<0){reply().println("ERR Wi-Fi scan failed; try again");return true;}
        reply().print("OK networks ");
        for(int i=0;i<count&&i<24;++i){if(i)reply().print(';');reply().printf("%s,%d,%d",hex(WiFi.SSID(i)).c_str(),WiFi.RSSI(i),WiFi.encryptionType(i)==WIFI_AUTH_OPEN?0:WiFi.encryptionType(i)==WIFI_AUTH_WPA2_ENTERPRISE?2:1);}
        reply().println();return true;
    }
    if(strncmp(line,"WIFI_JOIN ",10)==0){
        char ssid_hex[65],password_hex[129],extra;String ssid,password;
...
void poll(void (*command)(const char *)){
    ...
    if(udp.parsePacket()){
        char request[32]={};const int length=udp.read(request,sizeof(request)-1);
        if(length==14&&strcmp(request,"DECKY_DISCOVER")==0){udp.beginPacket(udp.remoteIP(),udp.remotePort());udp.printf("DECKY %s %d",serial.c_str(),PORT);udp.endPacket();}
    }
    if(!client.connected()){
        WiFiClient incoming=server.accept();
        if(incoming){
            client=incoming;client.setNoDelay(true);secure.reset();authenticated=false;client_at=millis();net_length=0;net_overflow=false;
    ...
    if(client.connected()){
        if(!authenticated){
            if(millis()-client_at>5000){drop_client();return;}
            read_handshake();
        } else {
